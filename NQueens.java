...
class NQueens {
    ...
    private void markForward (int row, int col, int step, 
    	...
    	{
    		if(row-step>=0&&board[row-step][col+step]==oldMarker)
    		{
    		...
    		if(row+step<BOARD_SIZE&&board[row+step][col+step]==oldMarker)
    		{
    			board[row+step][col+step]=newMarker;
    		}
    		markForward(row,col,++step,oldMarker,newMarker);
    	}
	// TO COMPLETE
    }
    /**
